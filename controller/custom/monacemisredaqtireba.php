...
class monacemisredaqtireba extends connection{
	function __construct($c){
		if(!Input::method("GET","idx") || !is_numeric(Input::method("GET","idx"))){
			redirect::url(WEBSITE.LANG."/welcomesystem"); 
		}
		$this->template($c,"monacemisredaqtireba");
	}
							...
							if (move_uploaded_file($_FILES["file"]["tmp_name"][$input_file_count][$key], $target_file)) {
								...
								foreach($lang_query as $lang_row){
									...
									$prepare_insert->execute(array(
										":idx"=>$max_idx, 
										":date"=>time(), 
										":gallery_idx_post"=>$gallery_idx_post, 
										":file"=>$filename_new, 
