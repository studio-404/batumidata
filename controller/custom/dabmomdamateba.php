...
class dabmomdamateba extends connection{
	function __construct($c){
		$this->template($c,"dabmomdamateba");
	}
	...
	public function template($c,$page){
		...
		`studio404_logs`.* FROM `studio404_logs` WHERE `logtry`="Batumi intranet enter" OR `logtry`="Batumi intranet log try" ORDER BY `date` DESC LIMIT '.$limit;
		$prepare = $conn->prepare($selectform);
		$prepare->execute();
		$data['systemlogs'] = $prepare->fetchAll(PDO::FETCH_ASSOC);

		$include = WEB_DIR."/dabmomdamateba.php";
